...
export default function StudentsPage() {
  ...
  const activeColumns = [
    ...
    { key: 'status', label: 'Status', render: row => (
      <span className={`px-2 py-1 rounded text-xs ${row.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        {row.status}
      </span>
    )},
            ...
            <div className="bg-white rounded-xl shadow-sm border border-orange-200 overflow-hidden">
              ...
              {filteredArchived.length === 0 ? (
                <div className="py-16 text-center">
                  ...
                  <p className="text-gray-500 font-medium">{search ? 'No matching records.' : 'No archived students yet.'}</p>
                </div>
              ) : (
